...
void CSoundSystem::Init() {
    ...
    auto getAsset = [&](const char* path) {
        std::string fullPath = appDir + path;
        if (!FileExists(fullPath.c_str())) return std::string(path);
        return fullPath;
    };
    LoadActionSound("footstep_walk", getAsset("assets/sounds/footstep_walk.mp3").c_str());
    LoadActionSound("footstep_run", getAsset("assets/sounds/footstep_run.mp3").c_str());
    LoadActionSound("white_noise", getAsset("assets/sounds/white_noise.wav").c_str());
    LoadActionSound("cosmic_noise", getAsset("assets/sounds/cosmic_noise.wav").c_str());
    LoadActionSound("click", getAsset("assets/sounds/click.wav").c_str());
    LoadActionSound("scream", getAsset("assets/sounds/scream.mp3").c_str());
    LoadActionSound("mumbling", getAsset("assets/sounds/mumbling.wav").c_str());
}
