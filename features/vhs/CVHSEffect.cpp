...
void main()
{
    ...
    // Flashlight multiplier: 1.2 in the center (bright, but not blown out), down to 0.05 at edges
    float lightMultiplier = mix(0.005, 1.00, spotCone);
    
    if (flashlightOn < 0.5) {
        lightMultiplier = 0.02; // Intense dark horror ambient when flashlight is OFF
    }
